...
func TestTC(t *testing.T) {
	g := New("Not.And.Or.Not.And.And.d0.d1.d1.d1.d0.d1.d1.c0", functions.Bool)
	fmt.Println(g.IsTerminal(g.Symbols[0]), g.Symbols[0])
	if g.IsTerminal(g.Symbols[0]) != false {
		t.Errorf("TestTC for Terminal failed: %s\n", g.Symbols[0])
	}
	fmt.Println(g.IsTerminal(g.Symbols[7]), g.Symbols[7])
	if g.IsTerminal(g.Symbols[7]) != true {
		t.Errorf("TestTC for Terminal failed: %s\n", g.Symbols[7])
	}
	fmt.Println(g.IsConstant(g.Symbols[13]), g.Symbols[13])
	if g.IsConstant(g.Symbols[13]) != true {
		t.Errorf("TestTC for Constant failed: %s\n", g.Symbols[13])
	}
}
