...
func (b *NextMode) Run() {
	...
	for {
		...
		case d, ok := <-b.ch.DataChan:
			...
			// 判断时间，如果时间大于下午3点，就认为需要结算了
			if d.Key.After(b.nextSettleTime) {
				b.currTime = b.nextSettleTime

				config.DebugF("结算时间：%v", b.currTime)
...
func (b *NextMode) Init(resource setting.Resource, options ...setting.WithResource) error {
	...
	b.trainMode = b.Config().Mode == config.TrainMode
	...
	b.matcher = &matcher2.NextQuote{
		StockSlippage: b.Config().Framework.Stock.Slippage, FutureSlippage: b.Config().Framework.Future.Slippage,
	}
...
func init() {
	setting.RegisterRTExecutor(&NextMode{}, config.HandlerTypeNextMatch)
}
