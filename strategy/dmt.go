...
type DMTStockInfo struct {
	lastTradeSignal float64 // 公式上一次买入信号数值
...
type DMT struct {
	...
	stockInfos map[string]*DMTStockInfo // 股票信息
...
func (s *DMT) OnInitialize(framework handler.Framework) {
	s.acc = framework.Account().GetAccountByID(config.AccountTypeStockSimple)
	s.stockInfos = make(map[string]*DMTStockInfo, len(s.SInstNames))
	...
	for _, name := range s.SInstNames {
		s.stockInfos[name] = new(DMTStockInfo)
		// 账户初始资金充当市值情况
		s.stockInfos[name].fa = average
		s.stockInfos[name].lastTradeSignal = -1
...
func (s *DMT) OnDailyOpen(framework handler.Framework, marketType config.MarketType, acc ...account.Account) {
...
func (s *DMT) buy(instID string, qty, price float64) bool {
	// 买入
...
func (s *DMT) sell(instID string, qty, price float64) bool {
	// 卖出
	o, err := s.acc.NewOrder(
		instID, qty,
		account.WithOrderPrice(price),
...
func (s *DMT) ContainNaN(m dataframe.StreamingRecord) bool {
...
func (s *DMT) OnTick(
	...
	dealingFunc := func(
		instID string, stockInfo *DMTStockInfo, record ...*dataframe.StreamingRecord,
	) {
		// if stockInfo.triggerState == 0 && tm.Compare(s.CurrTriggerTime) >= 0 {
		// 	stockInfo.triggerState = 1
		// } else {
		// 	return
		// }

		// if y, m, d := tm.Date(); y == 2020 && m == 05 && d == 13 {
		// 	fmt.Println("instID is: ", instID)
		// }
		...
		if !s.ContainNaN(*indicate) {
			...
			nowSignal := tradeSignal[0]
			lastSignal := stockInfo.lastTradeSignal

			// if stockInfo.triggerState == 1 {
			if nowSignal > 0 && lastSignal < 0 {
			...
			} else if nowSignal < 0 && lastSignal > 0 {
			...
			stockInfo.lastTradeSignal = nowSignal
		}
...
func (s *DMT) OnDailyClose(framework handler.Framework, acc map[string]account.Account) {
	// TODO  获取当日有复权的标的
	// TODO  重新记录信号值
	// if len(s.SInstNames) != 0 {
	// 	for _, instID := range s.SInstNames {
