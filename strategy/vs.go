...
func (s *VS) OnInitialize(framework handler.Framework) {
...
func (s *VS) OnDailyOpen(framework handler.Framework, marketType config.MarketType, acc ...account.Account) {
...
func (s *VS) buy(instID string, qty, price float64) bool {
	// 买入
...
func (s *VS) sell(instID string, qty, price float64) bool {
	// 卖出
	o, err := s.acc.NewOrder(
		instID, qty,
		account.WithOrderPrice(price),
