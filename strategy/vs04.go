...
// + Base conds set the Daily Range and GEP Sort & buy
...
type VS04 struct {
	...
	vsTargets	[]SortRank // VS条件设定总体集合
	tmpTargets  []SortRank // 临时目标集合，vsTargets的子集合
...
func (s *VS04) OnInitialize(framework handler.Framework) {
...
func (s *VS04) OnDailyOpen(framework handler.Framework, marketType config.MarketType, acc ...account.Account) {
...
func (s *VS04) OnTick(
	...
	if !s.bTriggerOnce {
		...
		// once the vsTargets is set, it will not be changed until the next VS base conditions are met
		if len(signals) != 0 {
			s.vsTargets = filterSignal(signals, func(sr SortRank) bool {
				return sr.close > 0.0
			})
		}
		// Given the vsTargets, we can now set the tmpTargets Daily!
		// Sort the vsTargets
		sort.Slice(
...
func (s *VS04) buy(instID string, qty, price float64) bool {
	// 买入
...
func (s *VS04) sell(instID string, qty, price float64) bool {
	// 卖出
	o, err := s.acc.NewOrder(
		instID, qty,
		account.WithOrderPrice(price),
