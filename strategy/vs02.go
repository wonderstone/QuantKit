...
func (s *VS02) OnInitialize(framework handler.Framework) {
...
func (s *VS02) OnDailyOpen(framework handler.Framework, marketType config.MarketType, acc ...account.Account) {
	...
	if !s.bTriggerOnce {
		...
		for pIndicate := indicators.Oldest(); pIndicate != nil; pIndicate = pIndicate.Next() {
			...
			if notNaN && notT0 {
				...
				_, errbp := indicate.TryConvertToFloat("bp")
				_, errnp := indicate.TryConvertToFloat("np")
				_, errdiv := indicate.TryConvertToFloat("div")
		...
		if len(signals) != 0 {
			amt = arithmeticSequence(s.acc.GetAsset().Total*s.fCashUsedRatio, len(s.tmpTargets), 1)
		}
...
func (s *VS02) buy(instID string, qty, price float64) bool {
	// 买入
...
func (s *VS02) sell(instID string, qty, price float64) bool {
	// 卖出
	o, err := s.acc.NewOrder(
		instID, qty,
		account.WithOrderPrice(price),
