...
func (tr *Map[K, V]) nodeSet(
	pn **mapNode[K, V], item mapPair[K, V],
...
func (tr *Map[K, V]) delete(
	pn **mapNode[K, V], max bool, key K,
) (mapPair[K, V], bool) {
	n := tr.isoLoad(pn, true)
	var i int
	var found bool
...
func (tr *Map[K, V]) nodeAscend(
	cn **mapNode[K, V], pivot K,
	iter func(key K, value V) bool, mut bool,
) bool {
	n := tr.isoLoad(cn, mut)
...
func (tr *Map[K, V]) nodeKeyValues(
	cn **mapNode[K, V], keys []K, values []V,
	mut bool,
) ([]K, []V) {
	n := tr.isoLoad(cn, mut)
	if n.leaf() {
