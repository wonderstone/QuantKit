...
func (tr *BTreeG[T]) find(
	n *node[T], key T, hint *PathHint, depth int,
...
func (tr *BTreeG[T]) nodeScan(
	cn **node[T], iter func(item T) bool, mut bool,
) bool {
	n := tr.isoLoad(cn, mut)
	if n.leaf() {
...
func (tr *BTreeG[T]) delete(
	cn **node[T], max bool, key T,
	hint *PathHint, depth int,
) (T, bool) {
...
func (tr *BTreeG[T]) ascend(
	pivot T, iter func(item T) bool, mut bool,
	hint *PathHint,
) {
	if tr.lock(mut) {
		defer tr.unlock(mut)
...
func (tr *BTreeG[T]) AscendHint(
	pivot T, iter func(item T) bool, hint *PathHint,
) {
	tr.ascend(pivot, iter, false, hint)
}
func (tr *BTreeG[T]) AscendHintMut(
	pivot T, iter func(item T) bool,
	hint *PathHint,
) {
...
func (tr *BTreeG[T]) nodeAscend(
	cn **node[T], pivot T, hint *PathHint,
	...
	if !found {
		if !n.leaf() {
			if !tr.nodeAscend(
				&(*n.children)[i], pivot, hint, depth+1, iter,
				mut,
			) {
				return false
			}
...
func (tr *BTreeG[T]) DescendHint(
	pivot T, iter func(item T) bool,
	hint *PathHint,
) {
	tr.descend(pivot, iter, false, hint)
}
func (tr *BTreeG[T]) DescendHintMut(
	pivot T, iter func(item T) bool,
	hint *PathHint,
) {
...
func NewGenericOptions[T any](
	less func(a, b T) bool, opts Options,
) *Generic[T] {
