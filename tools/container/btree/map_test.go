...
func testMapNewBTree() *MapG[testMapKind, testMapKind] {
	return NewMapG[testMapKind, testMapKind](2, func(a, b testMapKind) int { return a - b })
	// return new(MapG[testMapKind, testMapKind])
}
...
func (tr *MapG[K, V]) lt(a, b K) bool  { return tr.compareF(a, b) < 0 }
func (tr *MapG[K, V]) eq(a, b K) bool  { return tr.compareF(a, b) == 0 }
func (tr *MapG[K, V]) lte(a, b K) bool { return tr.compareF(a, b) <= 0 }
func (tr *MapG[K, V]) gt(a, b K) bool  { return tr.lt(b, a) }
func (tr *MapG[K, V]) gte(a, b K) bool { return tr.gt(a, b) || tr.eq(a, b) }
...
func TestMapDescend(t *testing.T) {
	...
	var count int
	tr.Descend(
		testMapMakeItem(rand.Int()), func(item, value testMapKind) bool {
			count++
			return true
		},
	)
	...
	var exp []testMapKind
	tr.Reverse(
		func(item, value testMapKind) bool {
			exp = append(exp, item)
			return true
		},
	)
	for i := 999; i >= 0; i-- {
		...
		var all []testMapKind
		tr.Descend(
			key, func(item, value testMapKind) bool {
				all = append(all, item)
				return true
			},
		)
...
func TestMapAscend(t *testing.T) {
	...
	var count int
	tr.Ascend(
		testMapMakeItem(1), func(item, value testMapKind) bool {
			count++
			return true
		},
	)
...
func TestMapKeyValues(t *testing.T) {
	...
	keys2 := tr.Keys()
	values2 := tr.Values()
	if !mapKindsAreEqual(keys, keys2) {
		t.Fatalf("not equal")
	}
	if !mapKindsAreEqual(values, values2) {
		t.Fatalf("not equal")
	}
...
func TestMapSimpleRandom(t *testing.T) {
	...
	for time.Since(start) < time.Second*2 {
		...
		tr.Ascend(
			pivot, func(item, value testMapKind) bool {
				if tr.lt(item, pivot) {
					panic("!")
				}
				return true
...
func TestMapRandom(t *testing.T) {
	...
	for i := 0; i < len(keys); i++ {
		...
		var j int
		tr.Ascend(
			keys[i], func(item, value testMapKind) bool {
				if j == 0 {
					res = item
				}
				j++
...
// btree_saneheight returns true if the height of all leaves match the height
// of the btree.
func (tr *MapG[K, V]) saneheight() bool {
	height := tr.Height()
	if tr.root != nil {
		if height == 0 {
			return false
...
func (n *mapNodeG[K, V]) saneheight(height, maxheight int) bool {
	if n.leaf() {
		if height != maxheight {
			return false
		}
...
// btree_deepcount returns the number of items in the btree.
func (tr *MapG[K, V]) deepcount() int {
	if tr.root != nil {
		return tr.root.deepcount()
	}
	return 0
...
func (n *mapNodeG[K, V]) deepcount() int {
	count := len(n.items)
	if !n.leaf() {
		for i := 0; i <= len(n.items); i++ {
			count += (*n.children)[i].deepcount()
...
func (tr *MapG[K, V]) nodesaneprops(n *mapNodeG[K, V], height int) bool {
	if height == 1 {
		if len(n.items) < 1 || len(n.items) > tr.max {
			println(len(n.items) < 1)
			return false
...
func (tr *MapG[K, V]) saneprops() bool {
	if tr.root != nil {
		return tr.nodesaneprops(tr.root, 1)
	}
	return true
...
func (tr *MapG[K, V]) sanenilsnode(n *mapNodeG[K, V]) bool {
	items := n.items[:cap(n.items):cap(n.items)]
	for i := len(n.items); i < len(items); i++ {
		if !tr.eq(items[i].key, tr.empty.key) {
			return false
...
//
// are equal to the empty value of the kind.
func (tr *MapG[K, V]) sanenils() bool {
	if tr.root != nil {
		return tr.sanenilsnode(tr.root)
	}
	return true
...
func (tr *MapG[K, V]) saneorder() bool {
	var last K
	var count int
	var bad bool
	tr.Scan(
...
func TestMapIterSeek(t *testing.T) {
	...
	_ = all
	{
		...
		require.True(t, vals[0] == 502 && vals[1] == 504, "")
	}
	{
		iter := tr.Iter()
		var vals []int
		...
		require.True(t, vals[0] == 502 && vals[1] == 500)
	}
}
...
func TestMapIterSeekPrefix(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		iter := tr.Iter()
		ret := iter.Seek(i*2 - 1)
		require.True(t, ret == true)
	}

	tr2 := NewMapG[int, struct{}](1, func(a, b int) int { return a - b })
	count2 := 10_000
	for i := 0; i < count2; i++ {
		tr2.Set(i*2, struct{}{})
	}
...
func TestMapValueCopy(t *testing.T) {
	t.Run(
		"without-copy", func(t *testing.T) {
			...
			v, _ = m.Get("hello")
			require.True(t, v.data == "planet")
			v, _ = m2.Get("hello")
			require.True(t, v.data == "planet")
		},
	)
	t.Run(
		"with-copy", func(t *testing.T) {
...
func TestMapDeepCopy(t *testing.T) {
	...
	require.True(t, count1 == Ncols*Nvals)
	require.True(t, count2 == Ncols*Nvals/2)
	...
	require.True(t, count1 == Ncols*Nvals/2)
	require.True(t, count2 == Ncols*Nvals/2)
}
